...
class ChatGptController(FlaskView):
    ...
    def GetFineTuneAnswer(self):
        ...
        answer = self.chatGptService.GetFineTuneAnswer(question, id, systemPrompt)
        response = {
            'message' : answer
        }
        return jsonify(response)
    ...
    def CreateFineTune(self):
        ...
        params = parser.parse_args()
        id = params["Id"]
        answer = self.chatGptService.CreateFineTune(id)
        response = {
            'message' : answer
        }
        return jsonify(response)
    ...
    def CreateJsonLFile(self):
        ...
        answer = self.jsonHelper.CsvToJsonl(fileName)
        response = {
            'message' : answer
        }
        return jsonify(response)
    ...
    def UploadFile(self):
        ...
        params = parser.parse_args()
        fileName = params["FileName"]
        answer = self.chatGptService.UploadFile(fileName)
